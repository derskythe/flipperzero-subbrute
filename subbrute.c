...
SubBruteState *subbrute_alloc() {
  ...
  instance->scene_manager =
      scene_manager_alloc(&subbrute_scene_handlers, instance);
  ...
  view_dispatcher_set_custom_event_callback(instance->view_dispatcher,
                                            subbrute_custom_event_callback);
  ...
  view_dispatcher_set_tick_event_callback(instance->view_dispatcher,
                                          subbrute_tick_event_callback, 100);
  ...
  view_dispatcher_add_view(instance->view_dispatcher, SubBruteViewTextInput,
                           text_input_get_view(instance->text_input));

  // Custom Widget
  instance->widget = widget_alloc();
  view_dispatcher_add_view(instance->view_dispatcher, SubBruteViewWidget,
                           widget_get_view(instance->widget));

  // VarList
  instance->var_list = variable_item_list_alloc();
  view_dispatcher_add_view(instance->view_dispatcher, SubBruteViewVarList,
                           variable_item_list_get_view(instance->var_list));

  // Popup
  instance->popup = popup_alloc();
  view_dispatcher_add_view(instance->view_dispatcher, SubBruteViewPopup,
                           popup_get_view(instance->popup));

  // ViewStack
  instance->view_stack = view_stack_alloc();
  view_dispatcher_add_view(instance->view_dispatcher, SubBruteViewStack,
                           view_stack_get_view(instance->view_stack));

  // SubBruteMainView
  instance->view_main = subbrute_main_view_alloc();
  view_dispatcher_add_view(instance->view_dispatcher, SubBruteViewMain,
                           subbrute_main_view_get_view(instance->view_main));

  // SubBruteAttackView
  instance->view_attack = subbrute_attack_view_alloc();
  view_dispatcher_add_view(
      instance->view_dispatcher, SubBruteViewAttack,
