 ...
 * - `SubBruteAttackAnsonic12bit433075`: Ansonic 12-bit 433.075 MHz sub-brute
 * attack.
 ...
 * - `SubBruteAttackHoltek12bitFM433`: Holtek 12-bit FM 433 MHz sub-brute
 * attack.
 * - `SubBruteAttackHoltek12bitAM433`: Holtek 12-bit AM 433 MHz sub-brute
 * attack.
 * - `SubBruteAttackHoltek12bitAM315`: Holtek 12-bit AM 315 MHz sub-brute
 * attack.
 * - `SubBruteAttackHoltek12bitAM868`: Holtek 12-bit AM 868 MHz sub-brute
 * attack.
 * - `SubBruteAttackHoltek12bitAM915`: Holtek 12-bit AM 915 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain9bit300`: Chamberlain 9-bit 300 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain9bit315`: Chamberlain 9-bit 315 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain9bit318`: Chamberlain 9-bit 318 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain9bit390`: Chamberlain 9-bit 390 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain9bit433`: Chamberlain 9-bit 433 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain8bit300`: Chamberlain 8-bit 300 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain8bit315`: Chamberlain 8-bit 315 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain8bit390`: Chamberlain 8-bit 390 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain7bit300`: Chamberlain 7-bit 300 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain7bit315`: Chamberlain 7-bit 315 MHz sub-brute
 * attack.
 * - `SubBruteAttackChamberlain7bit390`: Chamberlain 7-bit 390 MHz sub-brute
 * attack.
 ...
 * - `SubBruteAttackLinearDelta8bit310`: Linear Delta 8-bit 310 MHz sub-brute
 * attack.
 ...
 *
 * The SubBruteProtocol struct contains information about the protocol
 * parameters such as frequency, bits, transmission time (te), repeat count,
 * preset type, and file details using SubBruteFileProtocol.
 */
typedef struct {
  uint32_t frequency;
  uint8_t bits;
 ...
 *
 * This function returns the SubBruteProtocol object based on the provided
 * index.
 *
 ...
 *
 * This function takes a SubBruteFileProtocol and returns the corresponding file
 * protocol used by SubBrute.
 *
 ...
 *
 * This function takes a preset name as a FuriString pointer and converts it to
 * a corresponding FuriHalSubGhzPreset value. The converted preset is returned
 * as a FuriHalSubGhzPreset value. If the preset name is not recognized, the
 ...
 *
 * This function returns the number of protocol repeats for a specific SubBrute
 * attack, identified by its index.
 *
 ...
 *
 * This function returns the protocol name associated with the specified
 * SubBrute attack.
 *
 * @param index The index of the SubBrute attack.
 ...
 *
 * This function performs a sub-brute force attack using the default payload on
 * the specified stream and file protocol. It continues the attack in steps,
 * with each step increasing the payload by a specified number of bits.
 *
 ...
 */
void subbrute_protocol_default_payload(Stream *stream,
                                       SubBruteFileProtocol file, uint64_t step,
 ...
 * @param two_bytes A boolean indicating whether to use two bytes in the
 * operation.
 *
 * @return None
 */
void subbrute_protocol_file_payload(Stream *stream, uint64_t step, uint8_t bits,
                                    uint32_t te, uint8_t repeat,
                                    uint8_t bit_index, uint64_t file_key,
                                    bool two_bytes);

/**
 * @brief Generates a file using the SubBrute protocol with default settings.
 *
 * This function generates a file using the SubBrute protocol with default
 * settings.
 *
 ...
 */
void subbrute_protocol_default_generate_file(Stream *stream, uint32_t frequency,
                                             FuriHalSubGhzPreset preset,
                                             SubBruteFileProtocol file,
                                             uint64_t step, uint8_t bits,
                                             uint32_t te);

/**
 * @brief Generates a file for the SubBrute protocol with the given parameters.
 *
 * This function generates a file for the SubBrute protocol using the provided
 * parameters. The generated file can be used for various purposes, such as
 * testing or data analysis.
 *
 ...
 * @param bits The number of bits to use for each transmission in the SubBrute
 * protocol.
 ...
 * @param two_bytes Indicates whether two bytes should be used for each
 * transmission (true) or not (false).
 */
void subbrute_protocol_file_generate_file(Stream *stream, uint32_t frequency,
                                          FuriHalSubGhzPreset preset,
                                          SubBruteFileProtocol file,
                                          uint64_t step, uint8_t bits,
                                          uint32_t te, uint8_t bit_index,
                                          uint64_t file_key, bool two_bytes);

/**
 * @brief Calculates the maximum value based on the attack type, number of bits,
 * and whether two bytes are used.
 *
 * This function calculates the maximum value that can be generated based on the
 * specified attack type, number of bits, and whether two bytes are used. The
 * result is returned as a 64-bit unsigned integer.
 *
 ...
 *
 * @return The maximum value that can be generated based on the attack
 * parameters (uint64_t).
 */
uint64_t subbrute_protocol_calc_max_value(SubBruteAttacks attack_type,
                                          uint8_t bits, bool two_bytes);
