 ...
 *
 * The SubBruteWorker class is responsible for performing sub-brute forcing
 * tasks. It manages the state, configuration and execution of the sub-brute
 * forcing algorithm.
 */
 ...
 *
 * The sub-brute worker thread performs a sub-brute force operation based on the
 * given context. It takes the context as an input, processes the data, and
 * returns a status code. The function is used as an entry point for the worker
 * thread.
 */
 ...
 *
 * This function is used to send a callback for the SubBruteWorker instance. The
 * callback is triggered when certain conditions are met during the worker's
 * execution.
 *
