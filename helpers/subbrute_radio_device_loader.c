 ...
 * The function uses a @c `while` loop with a maximum of 5 attempts to enable
 * the OTG power. If it succeeds, it will break out of the loop. If the maximum
 * number of attempts is reached and the OTG power is not enabled, the function
 * checks the USB voltage using @c `furi_hal_power_get_usb_voltage()`.
 *
 * If the voltage is below 4.5 volts, an error message is logged using @c
 * `FURI_LOG_E()` with a message includes the value of @c
 * `furi_hal_power_check_otg_fault()` (converted to 1 if true, 0 if false),
...
static void subbrute_radio_device_loader_power_on() {
  ...
  if (attempts == 0) {
    if (furi_hal_power_get_usb_voltage() < 4.5f) {
      FURI_LOG_E(TAG, "Error power otg enable. BQ2589 check otg fault = %d",
 ...
 *
 * This function checks if OTG (On-The-Go) power is enabled, and if so, it
 * disables it.
 *
 * @note This function is static and used internally by the radio device loader
 * module.
 *
 */
 ...
 *
 * This function checks if a radio device with the specified name is connected
 * externally. If the OTG power is not enabled, it attempts to power on the
 * radio device loader. It then retrieves the device information using the
 * provided name and checks if it is connected. Finally, if the OTG power was
 * initially disabled, it powers off the radio device loader.
 *
...
const SubGhzDevice *
subbrute_radio_device_loader_set(const SubGhzDevice *current_radio_device,
