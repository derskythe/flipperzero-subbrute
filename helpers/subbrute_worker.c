...
SubBruteWorker *subbrute_worker_alloc(const SubGhzDevice *radio_device) {
  ...
  subghz_environment_set_protocol_registry(instance->environment,
                                           (void *)&subghz_protocol_registry);
...
bool subbrute_worker_init_default_attack(SubBruteWorker *instance,
  ...
  instance->max_value = subbrute_protocol_calc_max_value(
      instance->attack, instance->bits, instance->two_bytes);
...
#ifdef FURI_DEBUG
  FURI_LOG_I(TAG,
             "subbrute_worker_init_default_attack: %s, bits: %d, preset: %s, "
             "file: %s, te: %ld, repeat: %d, max_value: %lld",
             subbrute_protocol_name(instance->attack), instance->bits,
             subbrute_protocol_preset(instance->preset),
             subbrute_protocol_file(instance->file), instance->te,
             instance->repeat, instance->max_value);
#endif
...
#ifdef FURI_DEBUG
  FURI_LOG_I(TAG,
             "subbrute_worker_init_file_attack: %s, bits: %d, preset: %s, "
             "file: %s, te: %ld, repeat: %d, max_value: %lld, key: %llX",
             subbrute_protocol_name(instance->attack), instance->bits,
             subbrute_protocol_preset(instance->preset),
             subbrute_protocol_file(instance->file), instance->te,
             instance->repeat, instance->max_value, instance->file_key);
#endif
...
bool subbrute_worker_start(SubBruteWorker *instance) {
  ...
  if (instance->state != SubBruteWorkerStateReady &&
    ...
    FURI_LOG_W(TAG, "Worker cannot start, invalid device state: %d",
               instance->state);
    return false;
  }
...
bool subbrute_worker_transmit_current_key(SubBruteWorker *instance,
  ...
  if (instance->state != SubBruteWorkerStateReady &&
      instance->state != SubBruteWorkerStateFinished) {
    FURI_LOG_W(TAG, "Invalid state for running worker! State: %d",
               instance->state);
    return false;
  }

  uint32_t ticks = furi_get_tick();
  if ((ticks - instance->last_time_tx_data) <
      SUBBRUTE_MANUAL_TRANSMIT_INTERVAL) {
#ifdef FURI_DEBUG
    FURI_LOG_D(TAG, "Need to wait, current: %ld",
               ticks - instance->last_time_tx_data);
#endif
    return false;
  }
  ...
  if (instance->attack == SubBruteAttackLoadFile) {
    subbrute_protocol_file_payload(stream, step, instance->bits, instance->te,
                                   instance->repeat, instance->load_index,
                                   instance->file_key, instance->two_bytes);
  } else {
    subbrute_protocol_default_payload(stream, instance->file, step,
                                      instance->bits, instance->te,
                                      instance->repeat);
  }
...
bool subbrute_worker_can_manual_transmit(SubBruteWorker *instance) {
  ...
  return !instance->worker_running &&
         ...
         ((furi_get_tick() - instance->last_time_tx_data) >
          SUBBRUTE_MANUAL_TRANSMIT_INTERVAL);
}
...
void subbrute_worker_subghz_transmit(SubBruteWorker *instance,
  ...
  subghz_devices_set_frequency(instance->radio_device,
                               instance->frequency); // TODO is freq valid check

  if (subghz_devices_set_tx(instance->radio_device)) {
    subghz_devices_start_async_tx(instance->radio_device,
...
int32_t subbrute_worker_thread(void *context) {
  ...
  while (instance->worker_running) {
    stream_clean(stream);
    if (instance->attack == SubBruteAttackLoadFile) {
      subbrute_protocol_file_payload(stream, instance->step, instance->bits,
                                     instance->te, instance->repeat,
                                     instance->load_index, instance->file_key,
                                     instance->two_bytes);
    } else {
      subbrute_protocol_default_payload(stream, instance->file, instance->step,
                                        instance->bits, instance->te,
                                        instance->repeat);
    }
#ifdef FURI_DEBUG
  ...
  instance->state = local_state == SubBruteWorkerStateTx
