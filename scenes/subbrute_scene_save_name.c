...
void subbrute_scene_save_name_on_enter(void *context) {
  ...
  if (instance->device->attack == SubBruteAttackLoadFile) {
    name_generator_make_auto(
        instance->text_store, sizeof(instance->text_store),
  ...
  text_input_set_result_callback(text_input, subbrute_text_input_callback,
                                 instance, instance->text_store,
                                 SUBBRUTE_MAX_LEN_NAME, true);
  ...
  text_input_set_validator(text_input, validator_is_file_callback,
                           validator_is_file);
...
bool subbrute_scene_save_name_on_event(void *context, SceneManagerEvent event) {
  ...
  if (event.type == SceneManagerEventTypeBack) {
    ...
    if (strcmp(instance->text_store, "") != 0) {
      furi_string_reset(instance->file_path);
      furi_string_cat_printf(instance->file_path, "%s/%s%s", SUBBRUTE_PATH,
                             instance->text_store, SUBBRUTE_FILE_EXT);
