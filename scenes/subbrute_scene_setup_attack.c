...
static void subbrute_scene_setup_attack_callback(SubBruteCustomEvent event,
                                                 void *context) {
  furi_assert(context);
...
static void
subbrute_scene_setup_attack_device_state_changed(void *context,
...
void subbrute_scene_setup_attack_on_enter(void *context) {
...
#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "Enter Attack: %s",
             subbrute_protocol_name(instance->device->attack));
#endif

  subbrute_worker_set_callback(instance->worker,
                               subbrute_scene_setup_attack_device_state_changed,
                               context);
  ...
  subbrute_attack_view_init_values(
      view, instance->device->attack, instance->device->max_value,
      instance->device->current_step, false,
...
bool subbrute_scene_setup_attack_on_event(void *context,
  ...
  if (event.type == SceneManagerEventTypeCustom) {
    if (event.event == SubBruteCustomEventTypeTransmitStarted) {
      ...
      subbrute_attack_view_init_values(view, instance->device->attack,
      ...
      if (subbrute_worker_can_manual_transmit(instance->worker)) {
        // MANUAL Transmit!
        // Blink
        notification_message(instance->notifications,
                             &sequence_blink_green_100);
        subbrute_worker_transmit_current_key(instance->worker,
    ...
    if (subbrute_worker_is_running(instance->worker)) {
      instance->device->current_step =
          subbrute_worker_get_step(instance->worker);
    }
