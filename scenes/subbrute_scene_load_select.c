...
void subbrute_scene_load_select_callback(SubBruteCustomEvent event,
                                         void *context) {
  furi_assert(context);
...
void subbrute_scene_load_select_on_enter(void *context) {
  ...
  subbrute_main_view_set_callback(view, subbrute_scene_load_select_callback,
                                  instance);
  subbrute_main_view_set_index(view, 7, instance->settings->repeat_values, true,
...
bool subbrute_scene_load_select_on_event(void *context,
  ...
  if (event.type == SceneManagerEventTypeCustom) {
    if (event.event == SubBruteCustomEventTypeIndexSelected) {
      ...
      instance->device->two_bytes =
          subbrute_main_view_get_two_bytes(instance->view_main);
      ...
      instance->device->max_value = subbrute_protocol_calc_max_value(
          instance->device->attack, instance->device->bit_index,
          instance->device->two_bytes);
      ...
      if (!subbrute_worker_init_file_attack(
              ...
              instance->device->file_protocol_info, total_repeats,
              instance->device->two_bytes)) {
