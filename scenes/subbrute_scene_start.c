...
void subbrute_scene_start_on_enter(void *context) {
  ...
  subbrute_main_view_set_callback(view, subbrute_scene_start_callback,
                                  instance);

  instance->device->attack = instance->settings->last_index;

  subbrute_main_view_set_index(view, instance->settings->last_index,
                               instance->settings->repeat_values, false,
                               instance->device->two_bytes, 0);

  view_dispatcher_switch_to_view(instance->view_dispatcher,
                                 instance->current_view);
}

void subbrute_scene_start_on_exit(void *context) { furi_assert(context); }
...
bool subbrute_scene_start_on_event(void *context, SceneManagerEvent event) {
  ...
  if (event.type == SceneManagerEventTypeCustom) {
    ...
    FURI_LOG_D(
        TAG,
        "Event: %ld, SubBruteCustomEventTypeMenuSelected: %s, "
        "SubBruteCustomEventTypeLoadFile: %s",
        event.event,
    ...
    if (event.event == SubBruteCustomEventTypeMenuSelected) {
      instance->settings->last_index =
          subbrute_main_view_get_index(instance->view_main);
      ...
      if ((subbrute_device_attack_set(instance->device,
                                      instance->settings->last_index,
                                      total_repeats) != SubBruteFileResultOk) ||
          (!subbrute_worker_init_default_attack(
              instance->worker, instance->settings->last_index,
              instance->device->current_step, instance->device->protocol_info,
