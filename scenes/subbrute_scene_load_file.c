...
void subbrute_scene_load_file_on_enter(void *context) {
  ...
  dialog_file_browser_set_basic_options(&browser_options, SUBBRUTE_FILE_EXT,
                                        &I_sub1_10px);
...
#else
  bool res = dialog_file_browser_show(instance->dialogs, load_path, app_folder,
                                      &browser_options);
#endif
#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "load_path: %s, app_folder: %s",
             furi_string_get_cstr(load_path), furi_string_get_cstr(app_folder));
#endif
  if (res) {
    load_result = subbrute_device_load_from_file(
        instance->device, furi_string_get_cstr(load_path));
    if (load_result == SubBruteFileResultOk) {
      instance->settings->last_index = SubBruteAttackLoadFile;
      subbrute_settings_set_repeats(
          instance->settings,
          subbrute_main_view_get_repeats(instance->view_main));
      uint8_t extra_repeats =
          subbrute_settings_get_current_repeats(instance->settings);
      ...
      if (load_result == SubBruteFileResultOk) {
        if (!subbrute_worker_init_file_attack(
                ...
                instance->device->bit_index, instance->device->key_from_file,
                instance->device->file_protocol_info, extra_repeats,
    ...
    if (load_result == SubBruteFileResultOk) {
      ...
      furi_string_cat_printf(dialog_msg, "Cannot parse\nfile: %s",
                             subbrute_device_error_get_desc(load_result));
      dialog_message_show_storage_error(instance->dialogs,
                                        furi_string_get_cstr(dialog_msg));
...
void subbrute_scene_load_file_on_exit(void *context) { UNUSED(context); }
