 ...
 *
 * This structure stores the settings for a sub-brute attack, such as an array
 * of repeat values and the last index used.
 */
typedef struct {
  uint8_t
      repeat_values[SubBruteAttackTotalCount]; /**< Array of repeat values */
 ...
 * @note The returned structure should be freed using subbrute_settings_free()
 * function.
 */
SubBruteSettings *subbrute_settings_alloc(void);

/**
 * @brief Frees the memory allocated for the SubBruteSettings instance and all
 * its members.
 *
 * This function should be used to release the memory allocated for an instance
 * of the SubBruteSettings structure. It ensures that all members of the
 * structure are properly freed.
 *
 ...
 *
 * This function loads the settings for the SubBrute instance and populates the
 * SubBruteSettings structure pointed to by the given instance.
 *
 * @param instance A pointer to the SubBruteSettings structure to populate with
 * the loaded settings.
 */
 ...
 *
 * This function is used to save the settings of the SubBrute instance to a
 * storage.
 *
 ...
 *       will depend on the implementation. It is the responsibility of the
 * caller to ensure that the instance is valid and has been initialized
 * properly. The caller should handle any errors that may occur during the
 * saving process.
 */
 ...
 *
 * This function allows you to set the value for a specific attack in the
 * SubBruteSettings instance.
 *
 * @param instance A pointer to the SubBruteSettings instance.
 ...
 */
void subbrute_settings_set_value(SubBruteSettings *instance,
                                 SubBruteAttacks index, uint8_t value);

/**
 * @brief Get the value of a specific attack setting in the SubBruteSettings
 * instance.
 *
 ...
 */
uint8_t subbrute_settings_get_value(SubBruteSettings *instance,
 ...
 */
void subbrute_settings_set_repeats(SubBruteSettings *instance,
                                   const uint8_t *repeated_values);

/**
 * @brief Gets the current number of repeats from the SubBruteSettings instance.
 *
 * This function retrieves the current number of repeats stored in the given
 * SubBruteSettings instance.
 *
 * @param instance Pointer to the SubBruteSettings instance.
