...
const SubBruteProtocol *subbrute_protocol_registry[] = {
    ...
    [SubBruteAttackAnsonic12bit433075] =
        &subbrute_protocol_ansonic_12bit_433075,
    ...
    [SubBruteAttackChamberlain9bit300] =
        &subbrute_protocol_chamberlain_9bit_300,
    [SubBruteAttackChamberlain9bit315] =
        &subbrute_protocol_chamberlain_9bit_315,
    [SubBruteAttackChamberlain9bit318] =
        &subbrute_protocol_chamberlain_9bit_318,
    [SubBruteAttackChamberlain9bit390] =
        &subbrute_protocol_chamberlain_9bit_390,
    [SubBruteAttackChamberlain9bit433] =
        &subbrute_protocol_chamberlain_9bit_433,
    [SubBruteAttackChamberlain8bit300] =
        &subbrute_protocol_chamberlain_8bit_300,
    [SubBruteAttackChamberlain8bit315] =
        &subbrute_protocol_chamberlain_8bit_315,
    [SubBruteAttackChamberlain8bit390] =
        &subbrute_protocol_chamberlain_8bit_390,
    [SubBruteAttackChamberlain7bit300] =
        &subbrute_protocol_chamberlain_7bit_300,
    [SubBruteAttackChamberlain7bit315] =
        &subbrute_protocol_chamberlain_7bit_315,
    [SubBruteAttackChamberlain7bit390] =
        &subbrute_protocol_chamberlain_7bit_390,
...
static const char *subbrute_key_file_start_with_tail =
    "Filetype: Flipper SubGhz Key File\nVersion: 1\nFrequency: %u\nPreset: "
    "%s\nProtocol: %s\nBit: %d\nKey: %s\nTE: %d\n";
static const char *subbrute_key_small_no_tail =
    "Bit: %d\nKey: %s\nRepeat: %d\n";
// static const char* subbrute_key_small_raw =
//     "Filetype: Flipper SubGhz Key File\nVersion: 1\nFrequency: %u\nPreset:
//     %s\nProtocol: %s\nBit: %d\n";
static const char *subbrute_key_small_with_tail =
    "Bit: %d\nKey: %s\nTE: %d\nRepeat: %d\n";
...
void subbrute_protocol_create_candidate_for_existing_file(FuriString *candidate,
                                                          uint64_t step,
                                                          size_t bit_index,
                                                          uint64_t file_key,
                                                          bool two_bytes) {
...
#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "file candidate: %s, step: %lld",
             furi_string_get_cstr(candidate), step);
#endif
}

void subbrute_protocol_create_candidate_for_default(FuriString *candidate,
...
#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "candidate: %s, step: %lld", furi_string_get_cstr(candidate),
             step);
#endif
}
...
#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "candidate: %s, step: %lld, repeat: %d, te: %s",
             furi_string_get_cstr(candidate), step, repeat,
             te ? "true" : "false");
#endif
  stream_clean(stream);
  if (te) {
    stream_write_format(stream, subbrute_key_small_with_tail, bits,
                        furi_string_get_cstr(candidate), te, repeat);
  } else {
    stream_write_format(stream, subbrute_key_small_no_tail, bits,
                        furi_string_get_cstr(candidate), repeat);
  }
...
void subbrute_protocol_default_generate_file(Stream *stream, uint32_t frequency,
  ...
  if (te) {
    stream_write_format(stream, subbrute_key_file_start_with_tail, frequency,
                        subbrute_protocol_preset(preset),
                        subbrute_protocol_file(file), bits,
                        furi_string_get_cstr(candidate), te);
  } else {
    stream_write_format(stream, subbrute_key_file_start_no_tail, frequency,
                        subbrute_protocol_preset(preset),
                        subbrute_protocol_file(file), bits,
...
uint64_t subbrute_protocol_calc_max_value(SubBruteAttacks attack_type,
  ...
  } else if (attack_type == SubBruteAttackSMC532624bit330 ||
