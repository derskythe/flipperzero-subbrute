...
void subbrute_main_view_center_displayed_key(Canvas *canvas, uint64_t key,
                                             uint8_t index, bool two_bytes) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (!two_bytes && i == index) {
      ...
      canvas_draw_rbox(canvas, item_x - 1, ITEM_Y, ITEM_WIDTH + 1, ITEM_HEIGHT,
                       ITEM_FRAME_RADIUS);
      ...
      if (i == index) {
        canvas_set_color(canvas, ColorBlack);
        canvas_draw_rbox(canvas, item_x - ITEMS_INTERVAL - ITEM_WIDTH - 1,
                         ITEM_Y, ITEM_WIDTH * 2 + ITEMS_INTERVAL * 2 + 1,
                         ITEM_HEIGHT, ITEM_FRAME_RADIUS);
        ...
        snprintf(current_value, sizeof(current_value), "%02X", byte_value);
        canvas_draw_str(canvas, text_x - (TEXT_WIDTH + TEXT_INTERVAL), TEXT_Y,
                        current_value);
      } else {
        canvas_set_color(canvas, ColorWhite);
        canvas_draw_str(canvas, text_x, TEXT_Y, current_value);
...
void subbrute_main_view_draw_is_byte_selected(Canvas *canvas,
                                              SubBruteMainViewModel *model) {
  canvas_set_font(canvas, FontSecondary);
  canvas_draw_str_aligned(canvas, 64, 17, AlignCenter, AlignTop,
                          "Please select values to calc:");

  subbrute_main_view_center_displayed_key(canvas, model->key_from_file,
                                          model->index, model->two_bytes);
...
void subbrute_main_view_draw_is_ordinary_selected(
    Canvas *canvas, SubBruteMainViewModel *model) {
    ...
    if (item_position < ITEMS_ON_SCREEN) {
      if (model->index == position) {
        canvas_draw_str_aligned(
            canvas, 3,
            string_height_offset + (item_position * item_height) +
                STATUS_BAR_Y_SHIFT,
            AlignLeft, AlignCenter, subbrute_protocol_name(position));

        elements_frame(canvas, 1,
                       1 + (item_position * item_height) + STATUS_BAR_Y_SHIFT,
                       124, 15);
      } else {
        canvas_draw_str_aligned(
            canvas, 4,
      ...
      if (current_repeat_count > min_repeat_count) {
        ...
        canvas_draw_str_aligned(canvas, screen_width - temp_x_offset_repeats,
                                string_height_offset +
                                    (item_position * item_height) +
                                    STATUS_BAR_Y_SHIFT,
                                AlignLeft, AlignCenter, buffer);
        canvas_set_font(canvas, FontSecondary);
      }
    }
  }

  elements_scrollbar_pos(canvas, screen_width, STATUS_BAR_Y_SHIFT + 2,
                         screen_height - STATUS_BAR_Y_SHIFT, model->index,
...
bool subbrute_main_view_input_file_protocol(InputEvent *event,
...
bool subbrute_main_view_input_ordinary_protocol(InputEvent *event,
                                                SubBruteMainView *instance,
                                                bool is_short) {
  ...
  if (event->key == InputKeyUp && is_short) {
    ...
    if (index == SubBruteAttackLoadFile) {
      ...
      instance->callback(SubBruteCustomEventTypeMenuSelected,
                         instance->context);
    }
  ...
  if (updated) {
    ...
    if (SubBruteAttackTotalCount <= ITEMS_ON_SCREEN) {
      ...
      if (instance->window_position >=
          (SubBruteAttackTotalCount - ITEMS_ON_SCREEN)) {
        instance->window_position =
            (SubBruteAttackTotalCount - ITEMS_ON_SCREEN);
      }
    }
  }
...
bool subbrute_main_view_input(InputEvent *event, void *context) {
...
#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "InputKey: %d, extra_repeats: %d", event->key,
  ...
  if (instance->is_select_byte) {
    ...
    updated =
        subbrute_main_view_input_ordinary_protocol(event, instance, is_short);
  }
  ...
  if (updated) {
    with_view_model(
        instance->view, SubBruteMainViewModel * model,
        {
          ...
          model->repeat_values[model->index] =
              instance->repeat_values[instance->index];
        },
        true);
  }
...
void subbrute_main_view_enter(void *context) { furi_assert(context); }

void subbrute_main_view_exit(void *context) { furi_assert(context); }
...
SubBruteMainView *subbrute_main_view_alloc() {
  ...
  view_allocate_model(instance->view, ViewModelTypeLocking,
                      sizeof(SubBruteMainViewModel));
  view_set_context(instance->view, instance);
  view_set_draw_callback(instance->view,
                         (ViewDrawCallback)subbrute_main_view_draw);
