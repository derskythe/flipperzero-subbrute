...
bool subbrute_attack_view_input(InputEvent *event, void *context) {
  ...
  if (!instance->is_attacking) {
    if (event->type == InputTypeShort && event->key == InputKeyOk) {
      ...
      instance->callback(SubBruteCustomEventTypeTransmitStarted,
                         instance->context);
      update = true;
      ...
      instance->callback(SubBruteCustomEventTypeExtraSettings,
                         instance->context);
      update = true;
    } else if (event->key == InputKeyDown) {
      instance->callback(SubBruteCustomEventTypeTransmitCustom,
                         instance->context);
      update = true;
    } else if (event->type == InputTypeShort) {
      if (event->key == InputKeyLeft) {
        instance->callback(SubBruteCustomEventTypeChangeStepDown,
                           instance->context);
      } else if (event->key == InputKeyRight) {
        instance->callback(SubBruteCustomEventTypeChangeStepUp,
                           instance->context);
      }
      update = true;
    } else if (event->type == InputTypeRepeat) {
      if (event->key == InputKeyLeft) {
        instance->callback(SubBruteCustomEventTypeChangeStepDownMore,
                           instance->context);
      } else if (event->key == InputKeyRight) {
        instance->callback(SubBruteCustomEventTypeChangeStepUpMore,
                           instance->context);
      }
      update = true;
    }
    ...
    if ((event->type == InputTypeShort || event->type == InputTypeRepeat) &&
      ...
      instance->is_attacking = false;
      instance->callback(SubBruteCustomEventTypeTransmitNotStarted,
                         instance->context);
...
SubBruteAttackView *subbrute_attack_view_alloc() {
  ...
  view_allocate_model(instance->view, ViewModelTypeLocking,
                      sizeof(SubBruteAttackViewModel));
  ...
  view_set_draw_callback(instance->view,
                         (ViewDrawCallback)subbrute_attack_view_draw);
...
void subbrute_attack_view_free(SubBruteAttackView *instance) {
  ...
  with_view_model(
      instance->view, SubBruteAttackViewModel * model,
      { icon_animation_free(model->icon); }, false);
...
void subbrute_attack_view_set_current_step(SubBruteAttackView *instance,
  ...
  with_view_model(
      instance->view, SubBruteAttackViewModel * model,
      { model->current_step = current_step; }, true);
}
...
void subbrute_attack_view_init_values(SubBruteAttackView *instance,
  ...
  FURI_LOG_I(TAG,
             ...
             "extra_repeats: %d",
             index, max_value, current_step, extra_repeats);
#endif
...
void subbrute_attack_view_exit(void *context) {
  ...
  with_view_model(
      instance->view, SubBruteAttackViewModel * model,
      { icon_animation_stop(model->icon); }, false);
}
...
void subbrute_attack_view_draw(Canvas *canvas, void *context) {
  ...
  if (model->max_value > 9999) {
    ...
    canvas_draw_str_aligned(canvas, 5, y_frequency, AlignLeft, AlignTop,
                            buffer);
    ...
    canvas_draw_str_aligned(canvas, 107, y_frequency + 13, AlignRight,
                            AlignBottom, buffer);
  } else if (model->max_value <= 0xFF) {
    canvas_set_font(canvas, FontBigNumbers);
    snprintf(buffer, sizeof(buffer), "%03d/%03d", (int)model->current_step,
             (int)model->max_value);
    canvas_draw_str_aligned(canvas, 64, y_frequency, AlignCenter, AlignTop,
                            buffer);
  } else {
    canvas_set_font(canvas, FontBigNumbers);
    snprintf(buffer, sizeof(buffer), "%04d/%04d", (int)model->current_step,
             (int)model->max_value);
    canvas_draw_str_aligned(canvas, 64, y_frequency, AlignCenter, AlignTop,
                            buffer);
  }
  ...
  if (!model->is_attacking) {
    ...
    snprintf(
        buffer, sizeof(buffer), "x%d",
        model
            ->repeat_count); // +
                             // subbrute_protocol_repeats_count(model->attack_type));
    ...
    canvas_draw_icon_animation(canvas, x - icon_width_with_offset,
                               y - icon_v_offset, model->icon);
    // Progress bar
    // Resolution: 128x64 px
    float progress_value = (float)model->current_step / (float)model->max_value;
    elements_progress_bar(canvas, 8, 37, 110,
                          progress_value > 1 ? 1 : progress_value);
