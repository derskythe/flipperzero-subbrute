...
typedef void (*SubBruteMainViewCallback)(SubBruteCustomEvent event,
                                         void *context);
typedef struct SubBruteMainView SubBruteMainView;

/**
 * @brief Sets the callback function and context for the SubBruteMainView
 * instance.
 *
 * This function sets the callback function and context for the SubBruteMainView
 * instance. The callback function will be invoked when specific events occur in
 * the SubBruteMainView. The context parameter allows passing additional data to
 ...
 */
void subbrute_main_view_set_callback(SubBruteMainView *instance,
 ...
 * @note The caller is responsible for freeing the allocated memory using the
 * `subbrute_main_view_free` function.
 */
 ...
 *
 * This function frees all the memory allocated for the given SubBruteMainView
 * instance, including its internal data structures.
 *
 ...
 *
 * This function returns the view associated with the given SubBruteMainView
 * instance.
 *
...
void subbrute_main_view_set_index(SubBruteMainView *instance, uint8_t idx,
                                  const uint8_t *repeats, bool is_select_byte,
                                  bool two_bytes, uint64_t key_from_file);

/**
 * @brief Get the index of the current subview in the main view.
 *
 * This function retrieves the index of the current subview within the main
 * view. The current subview is represented by a SubBruteMainView instance.
 *
 ...
 *
 * This function returns the repeats of the main view in the SubBrute
 * application as a constant pointer to an array of uint8_t values.
 *
 * @param instance A pointer to the SubBruteMainView instance.
 * @return A constant pointer to an array of uint8_t values representing the
 * repeats of the main view.
 */
 ...
 *
 * @param instance The SubBruteMainView instance from which to retrieve two
 * bytes.
 *
 ...
 *
 * This function is used to exit the subbrute attack view, by performing any
 * necessary clean up operations and freeing up any allocated memory.
 *
 * @param context A pointer to the context of the subbrute attack view.
 *                This context contains all the necessary data and variables
 * required by the view. It should not be NULL.
 */
 ...
 * @param context     A pointer to the context data for the subbrute attack
 * view.
 * @return            Boolean indicating whether the input event was handled
 * successfully. True if the event was handled successfully, false otherwise.
 */
 ...
 *
 * This function is responsible for drawing the subbrute attack view on the
 * canvas. The provided canvas should already be initialized and in a valid
 * state.
 *
 * @param canvas   The canvas on which the view should be drawn.
 * @param context  A pointer to any additional context or data that may be
 * needed for drawing.
 */
