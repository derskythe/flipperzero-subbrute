 ...
 *
 * This enumeration defines a set of possible results that can occur when
 * performing a sub-brute force operation on a file. Each result represents a
 * specific error or condition that can occur during the operation.
 */
typedef enum {
 ...
 *
 * This structure contains information and state variables required for
 * performing a SubBrute attack.
 *
 */
typedef struct {
...
static char *const error_device_incorrect_frequency =
    "Missing or incorrect Frequency";
...
static char *const error_device_dynamic_protocol_unsupported =
    "Dynamic protocol unsupported";
 ...
 *
 * @param[in] radio_device The SubGhzDevice used to initialize the
 * SubBruteDevice.
 ...
 * After calling this function, the instance is no longer valid and should not
 * be used.
 *
...
/**
 * Saves a file with the specified key name using the given SubBruteDevice
 * instance.
 *
 ...
 *
 * This function returns a string that describes the given device error ID. The
 * error ID should be obtained from a SubBruteFileResult value. The returned
 * string provides additional information about the error, such as its cause or
 * possible solutions.
 *
 * @param error_id The device error ID for which to retrieve the description.
 *
 * @return A pointer to a constant string containing the description of the
 * device error.
 */
const char *subbrute_device_error_get_desc(SubBruteFileResult error_id);
SubBruteFileResult subbrute_device_attack_set(SubBruteDevice *context,
 ...
 */
uint8_t subbrute_device_load_from_file(SubBruteDevice *context,
...
/**
 * @brief Frees the memory allocated for the protocol information of a
 * SubBruteDevice instance.
 *
 * This function will deallocate the memory used by the protocol information of
 * a SubBruteDevice instance, including all the individual protocol objects and
 * their data.
 *
 ...
 *
 * This function sets the default values for a SubBruteDevice attack
 * configuration based on the specified default_attack type.
 *
