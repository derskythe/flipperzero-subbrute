...
bool subbrute_device_save_file(SubBruteDevice *instance,
  ...
  do {
    ...
    if (instance->attack == SubBruteAttackLoadFile) {
      subbrute_protocol_file_generate_file(
          stream, instance->file_protocol_info->frequency,
          instance->file_protocol_info->preset,
          instance->file_protocol_info->file, instance->current_step,
          instance->file_protocol_info->bits, instance->file_protocol_info->te,
          instance->bit_index, instance->key_from_file, instance->two_bytes);
    } else {
      subbrute_protocol_default_generate_file(
          stream, instance->protocol_info->frequency,
          instance->protocol_info->preset, instance->protocol_info->file,
          instance->current_step, instance->protocol_info->bits,
...
SubBruteFileResult subbrute_device_attack_set(SubBruteDevice *instance,
                                              SubBruteAttacks type,
                                              uint8_t extra_repeats) {
  furi_assert(instance);
#ifdef FURI_DEBUG
  FURI_LOG_D(TAG, "subbrute_device_attack_set: %d, extra_repeats: %d", type,
             extra_repeats);
#endif
  ...
  if (type != SubBruteAttackLoadFile) {
    ...
    if (!instance->decoder_result ||
      ...
      FURI_LOG_E(
          TAG,
          "Can't load SubGhzProtocolDecoderBase in phase non-file decoder set");
    } else {
    ...
    // Calc max value
    instance->max_value = subbrute_protocol_calc_max_value(
        instance->attack, instance->file_protocol_info->bits,
        instance->two_bytes);
#ifdef FURI_DEBUG
...
#ifdef FURI_DEBUG
  FURI_LOG_I(TAG,
             "subbrute_device_attack_set: %s, bits: %d, preset: %s, file: %s, "
             "te: %ld, repeat: %d, max_value: %lld",
             subbrute_protocol_name(instance->attack), bits,
             subbrute_protocol_preset(preset), subbrute_protocol_file(file), te,
             repeat, instance->max_value);
#endif

  return SubBruteFileResultOk;
}

uint8_t subbrute_device_load_from_file(SubBruteDevice *instance,
  ...
  do {
    ...
    if (!subghz_devices_is_frequency_valid(instance->radio_device,
                                           temp_data32)) {
    ...
    instance->file_protocol_info->preset =
        subbrute_protocol_convert_preset(temp_str);
    ...
    instance->file_protocol_info->file =
        subbrute_protocol_file_protocol_name(temp_str);
    ...
    if (!flipper_format_read_hex(fff_data_file, "Key", key_data,
                                 sizeof(uint64_t))) {
